...
const REGIONI=D.regioni.map((r,id)=>Object.freeze({id,nome:r.nome}));
const PROVINCE=D.province.map((p,id)=>Object.freeze({id,...p}));
const COMUNI=D.comuni.map((c,id)=>Object.freeze({id,...c}));
...
function risolvi(catastale='F205'){
  ...
  const provincia=PROVINCE[comune.provincia],regione=D.regioni[provincia.regione];
